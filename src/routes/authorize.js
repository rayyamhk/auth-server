...
async function authorize(req, res) {
  ...
  try {
    ...
    if (!accessBearerToken || accessBearerToken.split(' ').length !== 2) {
      ...
      return res.set(403).send('Unauthorized').end();
    }
    ...
    if (error.name === 'TokenExpiredError') {
      ...
      try {
        ...
        const accessToken = await jwt.sign(payload, process.env.JWT_PRIVATE_KEY, { expiresIn: '15s' });
        logger.info('Authorized: Access token refreshed');
        return res.set(200).json({ user, accessToken, refreshToken }).end();
